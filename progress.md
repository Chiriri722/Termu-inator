...
## 2026-08-31 — `v0.2.16` S22U stdio purity follow-up

- Identified `final-verify-manifest (7).json` as the sole downloaded manifest for commit `30eaa4a78ef5...`; its SHA-256 matches the supplied sidecar exactly.
- Preserved normalized 0700/0600 evidence at `../Termu-inator-device-artifacts/s22u-2026-08-31-v0216-stdio-stderr-fail-1/` without modifying the Downloads originals.
- Confirmed both Chromium and Firefox PASS, all artifact hashes and cleanup evidence PASS, and benchmark correctly remains skipped. The sole blocker is interactive stdio `stderr_bytes=38`; observer restart is clean at zero.
- Began a TDD diagnosis of the interactive-only stdio startup path. Code-graph search does not index the standalone verifier script, so direct literal inspection is limited to that file and its focused tests.
- Traced the 38-byte boundary to the unique static warning `Could not find main Firefox window ID\n`. Added connect-level regressions that require no warning when a usable owned BiDi session exists, while retaining the warning when both X11 window discovery and BiDi are unavailable; RED execution is next.
- The exact two-test RED run produced one intended failure: the BiDi-owned session still called the 38-byte warning once. The no-window/no-BiDi preservation case passed, proving the test distinguishes harmless protocol-only operation from a genuinely degraded startup.
- Moved the missing-window decision out of the discovery helper and behind BiDi attachment. A missing X11 window is now informational only when the owned BiDi session is usable; the original warning remains when neither control path is available.
- The exact two-test RED set is GREEN under Python 3.14 warning-as-error: the owned-BiDi case emits no warning, while the double-unavailable case emits the original bounded warning exactly once.
- All 44 Firefox native execution regressions pass under Python 3.14 warning-as-error, including delayed endpoint ownership, cancellation cleanup, BiDi navigation/observation, and native fallback contracts.
- The expanded device-focused native/BiDi/legacy-adapter set passes 79/79 under Python 3.14 warning-as-error.
- All 40 focused canonical verifier tests pass under Python 3.14 warning-as-error; the strict zero-stderr device gate itself remains unchanged.
- The first 411-test Python 3.14 discovery reproduced only the managed sandbox's established 15 local TCP/Unix bind errors. The identical approved run passes all 411 tests with eight optional-MCP skips.
- The complete warning-as-error matrix now passes 411/411 on Python 3.11.15, 3.12.13, and 3.14.7; Python 3.11/3.12 retain the established eight optional-MCP skips, while the pinned-MCP Python 3.14 run executes the complete surface.
- Three-interpreter warning-as-error compileall, `git diff --check`, `bash -n setup.sh`, and changed-Python 100-column checks all pass.
- Built from an empty private staging tree whose tracked bytes match the working checkout. The 280,608-byte pre-commit wheel is SHA-256 `420b6702e519a4e6e0705540e3e7421904eda415fa632d51cb4f8a4e6824d328` and passes canonical ZIP/RECORD/metadata/license/member/entrypoint checks plus 58-source binding at digest `3119361fceed96a74b7fecc4a41e5bb06e8df9987b836ec39e500bc12e0600e1`.
- A fresh Python 3.14 venv installs the exact wheel's MCP extra, selects MCP 1.29.0 and websockets 17.0.1, passes clean `pip check`, PEP 610 wheel hash/size provenance, four-entrypoint verification, 58 installed-source bytes, and site-packages-only origin checks.
- The first installed stdio harness stopped at a harness-only `async def` one-liner syntax error before launching product code. Its untouched private output root is reused with a correctly separated coroutine definition.
- Fresh-wheel interactive→observer stdio now passes exact 14/12 inventories, protocol `2025-11-25`, server `0.1.0a1`, zero-byte stderr in both runs, private live control sockets, socket removal after both exits, and an approved read-only process check with zero candidate survivors.
- Preserved the exact wheel, checksum, non-approval README, and deliberately non-executable Hermes placeholder at `../Termu-inator-device-artifacts/s22u-2026-08-31-post-v0216-stdio-purity-candidate/`. Exact four-file inventory, wheel byte identity, checksum, 0700/0600 modes, unresolved commit locks, and absence of runnable Bash blocks all pass.
- Advanced Phase 7 to the clean-commit boundary: local repair authority is complete, benchmark remains closed, and executable Hermes instructions stay blocked until the user supplies the exact pushed five-file commit SHA.
- After the final plan/evidence updates, Python 3.14 again passes all 411 tests with eight optional-MCP skips; final diff whitespace, shell syntax, changed-Python width, and exact five-file worktree scope checks pass.

## 2026-09-04 — `v0.2.17` PASS intake and benchmark-authority hardening

- Revalidated local HEAD, clean worktree, exact five-file `v0.2.17` scope, Git tree, and live `origin/main` at `c4f580217c7f65b14ac635f17fcebc07870ea039`.
- Verified the downloaded canonical manifest checksum and preserved normalized 0700/0600 evidence outside the repository at `../Termu-inator-device-artifacts/s22u-2026-08-31-v0217-pass-1/`.
- Confirmed the manifest is a full S22U PASS: both backends, both zero-stderr MCP profiles, installation/source provenance, cleanup, and `benchmark_allowed=true`.
- Recorded Hermes' current-environment evidence that native cryptography advanced from canonical `50.0.0` to `50.0.1`; benchmark authority remains closed for the old output identity.
- Added benchmark authority tests before implementation. The intended REDs proved cryptography/Android drift was not checked, canonical manifest arguments were absent, output reuse was allowed, and CLI resolution could erase a manifest symlink.
- Added checksum/private-file loading, strict PASS/backend/stdio validation, current Git/runtime/wheel/install binding, same-venv executable validation, non-overwriting 0700 output creation, and safe authority fields in the sanitized benchmark report.
- Updated the Termux guide and its packaging contract to require the exact wheel and canonical manifest and to require a newly sealed output identity after environment drift.
- The first resume test command used unsupported macOS system Python 3.9 and the first compile gate hit the protected default pycache. Both are recorded in `task_plan.md`; subsequent tests use explicit supported interpreters and external cache paths.
- Managed sandbox discovery reproduced only the known 15 local socket bind denials. Approved identical runs now pass all 424 tests on Python 3.11.15, 3.12.13, and 3.14.7.
- Inspected the newly received benchmark pair. Its public summary exactly reproduces from the private raw report; both browsers meet all established operation targets. Preserved both files with the canonical manifest under the existing owner-private v0.2.17 evidence directory and added an exact aggregate checksum file without touching Downloads.
- Classified the benchmark as valid historical measurement evidence: it began about 98 seconds after canonical from the same checkout/venv, but the old report omitted canonical/wheel/cryptography authority and therefore cannot authorize a rerun after cryptography advanced to 50.0.1.
- Built and bound a 280,608-byte candidate wheel (`ae29b57995aee97d2957ff637aaaa1fe7edfd3aa6592bd973322cec58cb85b1a`) to all 58 packaged sources plus RECORD, metadata, licenses, members, and four entrypoints.
- A fresh constrained Python 3.14 venv installs the exact wheel, passes `pip check`, PEP 610 and installed-source binding, and the full 424-test suite without optional-MCP skips. The installed interactive/observer entrypoint runs expose 14/12 tools with protocol `2025-11-25`, zero stderr, private control sockets, and clean removal.
- Added and passed a venv-scoped distribution regression so ignored checkout metadata cannot override the installed wheel. Added and passed a second authority check after measurement; any mid-run identity drift suppresses report publication.
- Added a sanitized v0.2.17 S22U compact benchmark report with exact baseline deltas, RSS, artifact hashes, and the historical-authority boundary. Marked only the now-proven fixture E2E, one-device canonical, and compact performance-measurement gates complete.
- Preserved the wheel, checksum, local-authority README, and deliberately non-executable Hermes placeholder at `../Termu-inator-device-artifacts/s22u-2026-09-04-post-v0217-benchmark-authority-candidate/` with 0700/0600 modes. It remains unsealed until the user supplies the exact pushed ten-path commit SHA.
- Final focused benchmark/packaging coverage passes 37/37. Three-interpreter compileall, diff whitespace, shell syntax, and added-Python-width checks pass after the closing-authority change.
