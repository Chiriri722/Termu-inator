...
| Navigation, URL, title, text, HTML, and JavaScript evaluation | **Emulated** through WebDriver BiDi plus native compatibility operations | **Supported** through CDP `Page` and `Runtime` methods | Firefox uses an explicit browser-owned loopback BiDi session for verified navigation metadata and bounded JSON-compatible JavaScript results when available. The DevTools-console/clipboard evaluator remains only as a compatibility fallback when no BiDi session was established. Shared page commands still call the CDP-shaped session interface. |
